...
# TODO: Update the path here replace crwzeitnews --> your project name
from crwzeitnews.spiders.indian_express import IndianExpressSpider
from crwzeitnews.test.helpers.constant import SITEMAP_URL, TEST_ARTICLES
from crwzeitnews.test.helpers.utils import get_article_content, online_response_from_url
...
class TestSitemap(unittest.TestCase):
    ...
    def _test_sitemap_results(self, sitemap_urls):
        for sitemap_url in sitemap_urls:
            article_urls = self.spider.parse_sitemap(
                online_response_from_url(sitemap_url.url)
            )
            for article_url in list(article_urls)[
                :1
            ]:  # Fetching only first article for testing
                self.spider.parse_sitemap_article(
                    online_response_from_url(article_url.url)
                )
        with self.subTest():
            self.assertGreater(
                len(self.spider.articles),
                0,
                "Crawler did not fetched single article form sitemap",
            )
        with self.subTest():
            self.assertIsInstance(
                self.spider.articles, list, "Sitemap Article format mismatch"
            )
        with self.subTest():
            self.assertIsInstance(
                self.spider.articles[0], dict, "Sitemap Article format mismatch"
            )
